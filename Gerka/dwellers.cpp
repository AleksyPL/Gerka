...
blacksmith::blacksmith(player gracz)
{
	...
	menu[1] = "WZMOCNIJ NAPIERåNIK";
	menu[2] = "WZMOCNIJ R KAWICE";
...
void shaman::everything_about_quests(player &gracz)
{
	if (gracz.quest_name == "" && gracz.quest_id == "")
	{
		if (quest_id_info() == "s00")
		{
			...
			fancy_text("By zrobiÊ magiczny amulet potrzebujÍ trzech ludzkich zÍbÛw",1);
			fancy_text("Zap≥acÍ ci za to 1000 sztuk z≥ota");
			if (dialog_box() == 1)
			{
				gracz.add_quest("Zdobπdü trzy ludzkie zÍby", "s00");
			}
			else
			{
	...
	{
		fancy_text("Wykonujesz juø jakieú zadanie, wrÛÊ pÛüniej");
		sound_rejection();
	}
	else if (gracz.quest_name != "" && gracz.quest_id == quest_id_info())
	{
		if (gracz.quest_complete == 1 && gracz.quest_failed == 0)
		{
			if (quest_id_info() == "s00")
			{
				fancy_text("DziekujÍ ci, z≥oto jest twoje");
				gracz.remove_usage_item("Ludzki zπb",3);
				quest_id_set("s01");
			}
		...
		{
			fancy_text("Tym razem ci siÍ nie uda≥o ale moøesz sprÛbowaÊ znowu");
			gracz.remove_quest();
		}
	}
}
