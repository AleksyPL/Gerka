...
table::table()
{
	...
	counter_selected_to_sell_usage = 0;
	counter_selected_to_sell_alchemy = 0;
	counter_selected_to_sell_forge = 0;
	info[0] = "";
	info[1] = "";
...
void table::load_merch_blacksmith(blacksmith &kowal, player &gracz)
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		else
		{
			for (int j = i + 1; j < 20; j++)
			{
				if (temp_table[20 + j] != "")
				{
					temp_table[20 + i] = temp_table[20 + j];
					temp_table_amount[20 + i] = temp_table_amount[20 + j];
					temp_table_price[20 + i] = temp_table_price[20 + j];
					temp_table[20 + j] = "";
					temp_table_amount[20 + j] = 0;
					temp_table_price[20 + j] = 0;
					break;
				}
			}
		}
...
void table::select_item_to_sell(int numer, player &gracz)
{
	numer = numer - 1;
	for (int i = 0; i < 20; i++)
	{
		if (temp_table[40 + i] == temp_table[numer])
		{
			...
			if (strona_2 == 0)
			{
				gracz.inventory_usage_amount[numer-60]--;
			}
			else if (strona_2 == 1)
			{
				gracz.inventory_crafting_amount[numer-60]--;
			}
			else if (strona_2 == 2)
			{
				gracz.inventory_crafting_amount[numer-40]--;
			}
			if (temp_table_amount[numer] == 0)
			{
				...
				if (strona_2 == 0)
				{
					gracz.inventory_usage[numer-60] = "";
					sort_player_field();
				}
				else if (strona_2 == 1)
				{
					gracz.inventory_crafting[numer-60] = "";
					sort_player_field();
				}
				else if (strona_2 == 2)
				{
					gracz.inventory_crafting[numer-40] = "";
					sort_player_field();
				}
			}
			break;
...
void table::delete_from_selected_to_sell(int numer, player &gracz, seller handlarz)
{
	...
	{
		gracz.add_usage_item(temp_table[numer], 0, temp_table_amount[numer], 1);
	}
	else if (handlarz.search_on_lists(temp_table[numer]) == 1)
	{
		gracz.add_crafting_alchemy_item(temp_table[numer], 0, temp_table_amount[numer], 1);
	}
	else if (handlarz.search_on_lists(temp_table[numer]) == 2)
	{
		gracz.add_crafting_forge_item(temp_table[numer], 0, temp_table_amount[numer], 1);
	}
...
void table::check_selected_to_sell_field(player gracz, seller handlarz)
{
	counter_selected_to_sell_usage = 0;
	counter_selected_to_sell_alchemy = 0;
	counter_selected_to_sell_forge = 0;
	for (int i = 0; i < 20; i++)
	{
		if (handlarz.search_on_lists(temp_table[40 + i]) == 0)
		{
			if (gracz.find_usage_item(temp_table[40 + i]) == 0)
			{
				counter_selected_to_sell_usage++;
			}
		}
		if (handlarz.search_on_lists(temp_table[40 + i]) == 1)
		{
			if (gracz.find_crafting_alchemy_item(temp_table[40 + i]) == 0)
			{
				counter_selected_to_sell_alchemy++;
			}
		}
		if (handlarz.search_on_lists(temp_table[40 + i]) == 2)
		{
			if (gracz.find_crafting_forge_item(temp_table[40 + i]) == 0)
			{
				counter_selected_to_sell_forge++;
			}
		}
	}
}
int table::can_be_selected(player gracz, seller handlarz, string nazwa)
{
	...
	check_selected_to_sell_field(gracz, handlarz);
	if (handlarz.search_on_lists(nazwa) == 0)
	{
		int pomoc = gracz.count_free_fields_usage() - (counter_selected_to_buy_usage + counter_selected_to_sell_usage);
		if (gracz.find_usage_item(nazwa) == 1)
		{
			return 1;
		}
		else if (pomoc > 0 && pomoc >= (counter_selected_to_buy_usage + counter_selected_to_sell_usage))
		{
			return 1;
		}
		else
...
void table::decline_transaction(player &gracz,seller handlarz)
{
	for (int i = 0; i < 20; i++)
	{
		if (temp_table[40 + i] != "" && temp_table_amount[40 + i] != 0)
		{
			if (handlarz.search_on_lists(temp_table[40 + i]) == 0)
			{
				gracz.add_usage_item(temp_table[40 + i],0, temp_table_amount[40 + i],1);
			}
			else if (handlarz.search_on_lists(temp_table[40 + i]) == 1)
			{
				gracz.add_crafting_alchemy_item(temp_table[40 + i], 0, temp_table_amount[40 + i], 1);
			}
			else if (handlarz.search_on_lists(temp_table[40 + i]) == 2)
			{
				gracz.add_crafting_forge_item(temp_table[40 + i], 0, temp_table_amount[40 + i], 1);
			}
		}
	}
}
